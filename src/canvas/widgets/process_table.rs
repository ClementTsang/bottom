...
impl ProcessTableWidget for Painter {
    ...
    fn draw_processes_table<B: Backend>(
        ...
        if let Some(process_widget_state) = app_state.proc_state.widget_states.get_mut(&widget_id) {
            let process_data: &[ConvertedProcessData] =
            ...
            let is_on_widget = widget_id == app_state.current_widget.widget_id
                && !process_widget_state.is_on_search;

            let position = get_start_position(
                num_rows,
                &process_widget_state.scroll_state.scroll_direction,
                &mut process_widget_state.scroll_state.previous_scroll_position,
                process_widget_state.scroll_state.current_scroll_position,
            ...
            let process_rows = sliced_vec.iter().map(|process| {
                let stringified_process_vec: Vec<String> = vec![
                    if process_widget_state.is_grouped {
            ...
            let mut pid_or_name = if process_widget_state.is_grouped {
                "Count"
            } else {
                "PID(p)"
            }
    ...
    fn draw_search_field<B: Backend>(
        ...
        if let Some(process_widget_state) = app_state.proc_state.widget_states.get_mut(&widget_id) {
            ...
            let chosen_text = if process_widget_state.is_grouped {
                grouped_search_text
            } else if process_widget_state
                .process_search_state
            ...
            let cursor_position = process_widget_state.get_cursor_position();
            let current_cursor_position = process_widget_state.get_char_cursor_position();
            ...
            let query = process_widget_state.get_current_search_query().as_str();
            ...
            let case_style = if !process_widget_state.process_search_state.is_ignoring_case {
                self.colours.currently_selected_text_style
            } else {
                self.colours.text_style
            };

            let whole_word_style = if process_widget_state
                .process_search_state
            ...
            let regex_style = if process_widget_state
                .process_search_state
            ...
            let case_text = format!(
                ...
                if !process_widget_state.process_search_state.is_ignoring_case {
                    "*"
                } else {
                    " "
                }
            ...
            let current_border_style = if process_widget_state
                .process_search_state
                .search_state
                .is_invalid_search
            {
