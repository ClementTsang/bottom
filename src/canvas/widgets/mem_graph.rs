...
use crate::{
    ...
    canvas::{
        Painter,
        components::timeseries::{GraphData, LegendConstraints, PercentTimeGraph},
    ...
    pub fn draw_memory_graph(
        ...
        if let Some(mem_state) = app_state.states.mem_state.widget_states.get_mut(&widget_id) {
            let hide_x_labels = should_hide_x_label(
                ...
                mem_state.timeseries_state.get_autohide_timer_mut(),
                draw_loc,
            );
            let graph_data = {
                let mut size = 1;
            ...
            PercentTimeGraph {
                display_range: mem_state.timeseries_state.current_display_time(),
                hide_x_labels,
