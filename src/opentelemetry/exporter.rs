...
use opentelemetry_sdk::{
    Resource,
    metrics::{
        PeriodicReader, SdkMeterProvider, reader::DefaultAggregationSelector,
...
impl OpenTelemetryExporter {
    ...
    pub async fn export_cpu_data(&self, cpu_data: &[CpuMetric]) -> Result<(), OpenTelemetryError> {
        ...
        for (core_idx, cpu) in cpu_data.iter().enumerate() {
            ...
            self.metrics
                .cpu_usage
                .record(cpu.usage_percent as f64, &labels);

            if let Some(temp) = cpu.temperature {
                self.metrics.cpu_temp.record(temp as f64, &labels);
            }
        }
    ...
    pub async fn export_memory_data(
        ...
        self.metrics
            .memory_usage
            .record(mem_data.used_bytes, labels);
        self.metrics
            .memory_total
            .record(mem_data.total_bytes, labels);

        if let (Some(swap_used), Some(swap_total)) =
            (mem_data.swap_used_bytes, mem_data.swap_total_bytes)
        {
    ...
    pub async fn export_network_data(
        &self, network_data: &[NetworkMetric],
    ) -> Result<(), OpenTelemetryError> {
    ...
    pub async fn export_disk_data(
        &self, disk_data: &[DiskMetric],
    ) -> Result<(), OpenTelemetryError> {
    ...
    pub async fn export_process_data(
        &self, process_data: &[ProcessMetric],
    ) -> Result<(), OpenTelemetryError> {
        ...
        self.metrics
            .process_count
            .record(process_data.len() as u64, &[]);

        // Export individual process metrics (be careful about cardinality)
        for process in process_data.iter().take(50) {
            ...
            self.metrics
                .process_cpu_usage
                .record(process.cpu_usage_percent as f64, &labels);
            self.metrics
                .process_memory_usage
                .record(process.memory_bytes, &labels);
        }
    ...
    pub async fn export_temperature_data(
        &self, temp_data: &[TemperatureMetric],
    ) -> Result<(), OpenTelemetryError> {
        ...
        for temp in temp_data {
            ...
            self.metrics
                .temperature
                .record(temp.temperature_celsius as f64, &labels);
        }
    ...
    pub async fn export_gpu_data(&self, gpu_data: &[GpuMetric]) -> Result<(), OpenTelemetryError> {
        ...
        for gpu in gpu_data {
            ...
            if let Some(temperature) = gpu.temperature_celsius {
                self.metrics
                    .gpu_temperature
                    .record(temperature as f64, &labels);
            }
        }
