...
use bottom::{
    ...
    options::{
        config::{create_or_get_config, get_config_path},
        get_color_scheme, get_widget_layout, init_app,
    },
...
fn main() -> Result<()> {
    ...
    let (widget_layout, default_widget_id, default_widget_type_option) = get_widget_layout(&config)
        .context("Found an issue while trying to build the widget layout.")?;

    // FIXME: Should move this into build app or config
    let styling = {
        let colour_scheme = get_color_scheme(&config)?;
