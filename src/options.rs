...
fn get_network_legend(
    ...
    let error = BottomError::ConfigError("cannot set a legend position. It have these values ['top', 'topleft', 'topright', 'left', 'right', 'bottom', 'bottomright', 'bottomleft', 'none'".to_string());
    if let Some(s) = matches.get_one::<String>("network_legend") {
        match s.to_ascii_lowercase().trim() {
            "none" | "" => Ok(None),
            position => Ok(Some(position.parse::<LegendPosition>().map_err(|_| error)?)),
        }
    } else if let Some(flags) = &config.flags {
        if let Some(legend) = &flags.network_legend {
            Ok(Some(legend.parse::<LegendPosition>().map_err(|_| error)?))
        } else {
